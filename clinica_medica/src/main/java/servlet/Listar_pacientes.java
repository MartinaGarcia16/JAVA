...
public class Listar_pacientes extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		LinkedList<ObraSocial> turnosObraSocial = new LinkedList<>();
		...
		if ("reservar".equals(opc)) { 
			try {
				especialidades = ctrl.getEspecialidad();
			} catch (SQLException e) {
				e.printStackTrace();
			}
			request.setAttribute("especialidades", especialidades);
			request.setAttribute("paciente", p);
			request.getRequestDispatcher("reservarTurno.jsp").forward(request, response); 
		}
		...
		if ("turnos_profesional".equals(opc)) {
			...
			request.setAttribute("obraSocial", turnosObraSocial);
			request.getRequestDispatcher("WEB-INF/turnosProfesionales.jsp").forward(request, response);
		}
